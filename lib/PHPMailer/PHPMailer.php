...
class PHPMailer {
  ...
  */
  ...
  public function envioCorreo($modulo) {
        switch ($modulo):
            case 'contafacil':
                $mailtoCC = "jennyhpinedo@yahoo.es"; //"soporte@sensoft.com.pe";
                $destinatarioCC = "Jenny";
                break;
        endswitch;
        $this->IsSMTP();
        $this->IsHTML(true);
        $this->SMTPAuth = true;
        $this->CharSet = "UTF-8";
        $this->Host = "cepefodes.org.pe";
        $this->Username = "notificaciones@cepefodes.org.pe";
        $this->Password = "7458u";
        $this->From = "notificaciones@cepefodes.org.pe";
        $this->FromName = "CEPEFODES";
        $this->ConfirmReadingTo = $mailtoCC;
        if ($_SERVER['HTTP_HOST'] != 'cepefodes.proyectos.sensoft') {
            $this->AddCC($mailtoCC, $destinatarioCC);
        }
    }
}
